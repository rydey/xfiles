...
async function megaParseLog(filePath: string, maxMessages: number = 1) {
  console.log(`🔍 MEGA PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Mega parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run mega-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await megaParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Mega parse failed:', error);
    process.exit(1);
  }
}
