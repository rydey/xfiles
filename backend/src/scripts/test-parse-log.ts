...
async function testParseLog(filePath: string, maxMessages: number = 5) {
  console.log(`🔍 TEST PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Test parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run test-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await testParseLog(logFilePath, maxMessages);
  } catch (error) {
