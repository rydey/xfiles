...
class FinalParseStep {
  ...
  async parseLogFile(filePath: string, maxMessages: number = 1) {
    console.log(`🔍 FINAL PARSE STEP: Starting parse of ${filePath} (max ${maxMessages} messages)`);
    ...
    for await (const line of rl) {
      ...
      if (isNewMessage && currentRecord.length > 0) {
        ...
        if (parsed) {
          ...
          // Process batch immediately for step-by-step debugging
  ...
  private parseMessageRecord(lines: string[], recordId: number): ParsedMessage | null {
    try {
      ...
      // Extract phone number and content from the rest
      const { phoneNumber, contentStart, nameStart } = this.extractPhoneAndContent(rest);
      console.log(`📞 PHONE: ${phoneNumber}, CONTENT_START: ${contentStart}, NAME_START: ${nameStart}`);
      
      // Find the time and contact name from the continuation line (Messages line)
      let timeStr = '';
      let contactName = '';
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (timeMatch) {
          ...
          // Extract contact name and content after time from the Messages line
          // Pattern: "Messages 05:07:40(UTC+0) Pension Hassan content..."
          // First, remove the "Messages" word and time part to get the rest
          const afterTime = line.replace(/Messages\s+\d{2}:\d{2}:\d{2}\(UTC\+0\)/, '').trim();
          console.log(`📝 AFTER TIME: ${afterTime}`);
          
          // Split by multiple spaces to separate contact name from content
          const parts = afterTime.split(/\s{2,}/); // Split by 2 or more spaces
          if (parts.length >= 2) {
            const nameFromMessages = parts[0].trim();
            const contentAfterTime = parts.slice(1).join(' ').trim();
            
            // Use name from Messages line
            if (nameFromMessages) {
              contactName = nameFromMessages;
            }
            ...
            // Fallback: extract content after time
      ...
      return {
        ...
        timestamp,
        phoneNumber,
        name: contactName || undefined, // Extract contact name from Messages line
  ...
  private extractPhoneAndContent(text: string): { phoneNumber?: string; contentStart: string; nameStart?: string } {
    ...
    if (fromMatch) {
      ...
      // For now, treat everything after phone number as content
      // Names are typically found in the Messages line, not the first line
      return { phoneNumber, contentStart: rest };
    }
    
    if (toMatch) {
      const phoneNumber = toMatch[1];
      const rest = toMatch[2]?.trim() || '';
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run final-parse-step <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  const parser = new FinalParseStep();
  
  try {
    await parser.parseLogFile(logFilePath, maxMessages);
    console.log('🎉 Final parse step completed!');
  } catch (error) {
    console.error('❌ Final parse step failed:', error);
    process.exit(1);
  } finally {
