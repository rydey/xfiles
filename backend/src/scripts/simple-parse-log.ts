...
async function simpleParseLog(filePath: string, maxMessages: number = 3) {
  console.log(`🔍 SIMPLE PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Simple parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  const maxMessages = parseInt(args[1]) || 3;
  
  if (!logFilePath) {
    console.error('❌ Please provide log file path');
    console.error('Usage: npm run simple-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await simpleParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Simple parse failed:', error);
    process.exit(1);
  }
}
