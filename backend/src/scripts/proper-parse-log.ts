...
class ProperParseLog {
  ...
  async parseLogFile(filePath: string, maxMessages: number = 100) {
    console.log(`🔍 PROPER PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run proper-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  const parser = new ProperParseLog();
  
  try {
    await parser.parseLogFile(logFilePath, maxMessages);
    console.log('🎉 Proper import completed!');
  } catch (error) {
    console.error('❌ Proper import failed:', error);
    process.exit(1);
  } finally {
