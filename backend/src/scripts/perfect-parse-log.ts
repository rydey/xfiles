...
async function perfectParseLog(filePath: string, maxMessages: number = 1) {
  console.log(`🔍 PERFECT PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Perfect parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run perfect-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await perfectParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Perfect parse failed:', error);
    process.exit(1);
  }
}
