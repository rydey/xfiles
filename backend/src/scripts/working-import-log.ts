...
class WorkingImportLog {
  ...
  async parseLogFile(filePath: string, maxMessages: number = 100) {
    console.log(`🔍 WORKING IMPORT: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  private parseMessageRecord(lines: string[], recordId: number): ParsedMessage | null {
    try {
      ...
      const { phoneNumber, contentStart } = this.extractPhoneAndContent(rest);
      console.log(`📞 PHONE: ${phoneNumber}, CONTENT_START: ${contentStart}`);
      ...
      return {
        ...
        timestamp,
        phoneNumber,
        name: undefined, // We don't extract names from this format
  ...
  private extractPhoneAndContent(text: string): { phoneNumber?: string; contentStart: string } {
    ...
    if (fromMatch) {
      ...
      const contentStart = fromMatch[2]?.trim() || ''; // This is the actual message content
      return { phoneNumber, contentStart };
    }
    
    if (toMatch) {
      const phoneNumber = toMatch[1];
      const contentStart = toMatch[2]?.trim() || ''; // This is the actual message content
      return { phoneNumber, contentStart };
    }
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run working-import-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
