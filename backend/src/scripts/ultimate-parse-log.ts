...
async function ultimateParseLog(filePath: string, maxMessages: number = 1) {
  console.log(`🔍 ULTIMATE PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Ultimate parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run ultimate-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await ultimateParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Ultimate parse failed:', error);
    process.exit(1);
  }
}
