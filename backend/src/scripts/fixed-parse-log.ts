...
class FixedLogParser {
  ...
  async parseLogFile(filePath: string, maxMessages: number = 100) {
    console.log(`🔍 FIXED PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run fixed-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  const parser = new FixedLogParser();
