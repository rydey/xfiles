...
async function minimalParseLog(filePath: string, maxMessages: number = 2) {
  console.log(`🔍 MINIMAL PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Minimal parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  const maxMessages = parseInt(args[1]) || 2;
  
  if (!logFilePath) {
    console.error('❌ Please provide log file path');
    console.error('Usage: npm run minimal-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await minimalParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Minimal parse failed:', error);
    process.exit(1);
  }
}
