...
async function ultimateWorkingParseLog(filePath: string, maxMessages: number = 1) {
  console.log(`🔍 ULTIMATE WORKING PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  if (currentRecord.length > 0 && messageCount < maxMessages) {
    console.log(`\n🔍 PROCESSING FINAL RECORD ${recordId}:`);
    console.log(`Raw lines:`, currentRecord);
  }

  console.log(`\n🎉 Ultimate working parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run ultimate-working-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await ultimateWorkingParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Ultimate working parse failed:', error);
    process.exit(1);
  }
}
