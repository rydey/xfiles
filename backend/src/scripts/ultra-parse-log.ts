...
async function ultraParseLog(filePath: string, maxMessages: number = 1) {
  console.log(`🔍 ULTRA PARSER: Starting parse of ${filePath} (max ${maxMessages} messages)`);
  ...
  console.log(`\n🎉 Ultra parse completed! Processed ${messageCount} messages.`);
}
...
async function main() {
  ...
  if (!logFilePath) {
    ...
    console.error('Usage: npm run ultra-parse-log <path-to-log.txt> [max-messages]');
    process.exit(1);
  }
  ...
  try {
    await ultraParseLog(logFilePath, maxMessages);
  } catch (error) {
    console.error('❌ Ultra parse failed:', error);
    process.exit(1);
  }
}
